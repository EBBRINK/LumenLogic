...
function herlaadVerloopschermen(brandId: string | null) {
  revalidatePath("/brand-management/price-lists");
  revalidatePath("/brand-management");
  if (brandId) revalidatePath(`/brand-management/${brandId}`);
  revalidatePath("/admin/brands");
}

/**
 * Zet de einddatum van één prijslijst vooruit en komt terug op /brand-management/price-lists met de
...
export async function extendPriceListAction(formData: FormData) {
  await bewaakNiveau("intern", "/brand-management/price-lists");
  ...
  redirect(`/brand-management/price-lists?${uitkomst.toString()}`);
}
