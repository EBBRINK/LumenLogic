...
export default async function MerkrelatieDetailPage({
  ...
  await bewaakRoute("/brand-management/[brandId]");
          ...
          <div className="mt-4 rounded-xl bg-card p-5 text-card-foreground ring-1 ring-foreground/10">
            ...
            <ul className="space-y-1">
              {openVoorstellen.map((upload) => (
                <li key={upload.id}>
                  <Link
                    href={`/brand-management/${row.id}/upload/${upload.id}`}
        ...
        {indicator === "verlopen" && (
          <div className="mb-4">
            <PriceListExpiryNotice
              ...
              // stond de gebruiker hier stil. /brand-management/price-lists is de plek waar die
