...
// op het oude `redirect(\`/brand-management/${brandId}\`)` — precies bevinding C8 opnieuw
...
test("de action stuurt de tellingen mee in de redirect-URL (en niet het kale merkpad)", async () => {
  ...
  expect(url.pathname).toBe(`/brand-management/${brandId}`);
...
test("dubbelklik: de tweede POST redirect met applied=already, niet met tellingen", async () => {
  ...
  expect(tweede.pathname).toBe(`/brand-management/${brandId}`);
