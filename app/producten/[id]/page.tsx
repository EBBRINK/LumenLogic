...
export default async function ProductPage({
  ...
  searchParams: Promise<{ prijsaanvraag?: string }>;
}) {
  const { id } = await params;
  const { prijsaanvraag } = await searchParams;
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 py-8 pb-32">
      <div className="mb-4">
        <Link href="/catalogus" className="text-sm text-muted-foreground hover:text-foreground">
          ← Terug naar catalogus
        </Link>
      </div>

      {prijsaanvraag === "verzonden" && (
        <div className="mb-6 rounded-lg border bg-muted/40 p-3 text-sm">
          Je prijsaanvraag staat klaar bij Brink. Je hoort van ons zodra de adviesprijs bekend is.
        </div>
      )}
