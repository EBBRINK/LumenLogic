...
test("createSubstitution: veld-voor-veld met bron 'merk-opgave' + prijsverschil als tekst", async () => {
  ...
  const dossier = await createDossier(db, { name: "Ziekenhuis Noord", phase: "awarded" });
...
test("createSubstitution: duurdere alternatief → 'Meerprijs' als tekst (nog steeds geen sortering)", async () => {
  ...
  const dossier = await createDossier(db, { name: "Kantoor Zuid", phase: "awarded" });
...
test("getSubstitution + listSubstitutions: identiteit uit de catalogus + samenvatting", async () => {
  const { db, ref, alt } = await seed();
  const dossier = await createDossier(db, { name: "School West", phase: "awarded" });
  const row = await createSubstitution(db, {
    dossierId: dossier.id,
    referenceProductId: ref,
    alternativeProductId: alt,
...
test("createSubstitution: onzichtbaar (verlopen) product geeft geen voorstel", async () => {
  const db = await createTestDb();
  const dossier = await createDossier(db, { name: "Verlopen", phase: "awarded" });
  const { productId: ref } = await seedBrandProduct(db, {
    brand: "XAL",
    name: "SASSO 100 CEIL",
    categoryPath: CAT,
...
test("systeemAlternatieven: N spots in een zone → voorstel voor één lijnsysteem", async () => {
  const db = await createTestDb();
  const dossier = await createDossier(db, { name: "Kliniek", phase: "awarded" });
