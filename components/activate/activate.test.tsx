...
// invoervakjes in twee groepen van vier, de foutstaat via aria-invalid, en dat de generieke
// afwijzing ("invalid") altijd dezelfde tekst toont — dat is precies wat account-enumeratie
// voorkomt (het scherm mag nooit verraden of een e-mailadres bestaat).
//
...
test("acht invoervakjes voor de PIN, in twee groepen van vier", async () => {
  ...
  // Geen enkel vakje begint fout gemarkeerd.
...
test("PIN-fout: generieke afwijzing, aria-invalid op alle acht vakjes, geen reden die het adres verraadt", async () => {
  const { container } = await renderServer(
    <Screen>
  ...
  // De foutmelding is gekoppeld aan het veld.
...
test("zwak wachtwoord: eigen melding bij het wachtwoordveld, andere tekst dan de PIN-afwijzing", async () => {
  await renderServer(
    <Screen>
