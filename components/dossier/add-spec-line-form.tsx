  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <form action={addLineAction} className="flex flex-col gap-3">
        ...
        <p className="text-sm font-medium">Losse regel</p>
        <div className="grid grid-cols-3 gap-2">
          <Input name="fixtureCode" required placeholder="Code (Lp301)" />
          <Input name="quantity" type="number" min="1" defaultValue="1" placeholder="Aantal" />
          <Input name="brandText" placeholder="Merk (XAL)" />
        </div>
      ...
      <form action={addCsvAction} className="flex flex-col gap-3">
        <input type="hidden" name="dossierId" value={dossierId} />
        <p className="text-sm font-medium">CSV-blok plakken</p>
        <textarea
          name="csv"
          rows={6}
          placeholder={"code, aantal, merk, type\nLp301, 12, XAL, SASSO 100\nLw201, 8, Wever & Ducré, SCAVA 1.0"}
